...
-- ============================================================
-- Purpose:
--     Read from RAW_CRYPTO_MARKET_DATA and produce a clean,
--     enriched staging table with derived columns.
--     Deduplication keeps only the latest record per coin.
-- ============================================================
...
FROM (
    SELECT
        ...
        -- NEW: daily price range in USD
        (HIGH_24H - LOW_24H)                    AS price_range_24h,

        -- NEW: daily price range as percentage
        CASE
            WHEN LOW_24H > 0
        ...
        -- NEW: volume to market cap ratio
        CASE
            WHEN MARKET_CAP > 0
        ...
        -- NEW: circulating supply percentage
        CASE
            WHEN TOTAL_SUPPLY > 0
        ...
        -- NEW: date only — used for GROUP BY in analytics
        ...
        -- ── Deduplication ────────────────────────────────────
        -- Keeps only the most recent record per coin
        -- so we always have the latest price per coin
        ROW_NUMBER() OVER (
            PARTITION BY ID
